...
<body>
	<h3>WELCOME TO Tonynews.com - ALBUM EDIT FORM</h3>
	<a href="javascript:void(0)" onclick="$('#light_tab').hide(); $('#fade_tab').hide();">Cancel</a>
			...
			<div id = "edit_album">
				<ul>
					...
					<?php foreach ($images_info as $k => $v) { ?>
						<li>Name: <input type="text"  name="img_name[]" value="<?php echo $v->name;?>" size="10" />  <input type="file" value = "" name="images[]" size="10" /> <img src="<?php echo base_url();?>uploads/<?php echo $album_info['name']."/".$v->images;?>" width="150" height="100" />   <a href ="<?php echo base_url();?>admin_2/images_manage/delete_one_image/<?php echo $v->id;?>" onclick="return confirm('Are u sure ?');">delete this !</a></li>
					<?php }?>
