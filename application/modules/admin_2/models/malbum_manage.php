...
class MAlbum_manage extends CI_Model {
	...
	function update_album_info($name_ascii, $count){
		...
		$this->db->where('name_ascii', $name_ascii)->update($this->table, $arr);
	}
	...
	function get_album_info_by_name($name_ascii){
		$res = $this->db->where('name_ascii', $name_ascii)->get($this->table)->row_array();
		return $res;
	}
	...
	function delete_album_by_name_ascii($name_ascii){
		$this->db->where('name_ascii', $name_ascii)->delete($this->table);
		return true;
	}
	
	
	function get_album_name($name_ascci){
		$res = $this->db->select('name')->where('name_ascii',$name_ascci)->get($this->table)->row_array();
		return $res;
	}
	...
	function decrease_amount_images_by_name($name, $count){
		...
		$this->db->where('name', $name)->update($this->table, $arr);
	}
